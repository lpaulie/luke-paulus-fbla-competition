...
def login_search():
  ...
  def login_search_submit():
    ...
    WITH login_counts AS (
      SELECT name, SUM(logincount) AS total_logins
      FROM students
      GROUP BY name
    )
    SELECT total_logins FROM login_counts WHERE name = ?
    """

    # execute the query with a parameter
    
    c.execute(sql_query, (student_login_name,))
    ...
    #if-else statement to create a label corresponding to if grades are found for student_login_name

    if result:
      total_logins = result[0]
      
      tk.Label(login_count_window, text = f"Grades for {student_login_name}: {total_logins}", **label_settings_2).place(x = 50, y = 30)
    else:
      
      tk.Label(login_count_window, text = f"No results found for {student_login_name}", **label_settings_2).place(x = 50, y = 30)
